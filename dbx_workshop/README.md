...
```
dbx_workshop        <- Root directory. Both monorepo and polyrepo are supported.
│
├── dbx_workshop       <- Contains python code, notebooks and ML resources related to one ML project. 
│   │
...
* An automated integration test is provided (in `.github/workflows/dbx_workshop-run-tests.yml`) that executes a multi task run on Databricks involving the feature engineering and model training notebooks.
...
* Run `databricks configure --profile dbx_workshop-dev --token --host <your-dev-workspace-url>`, passing the URL of your dev workspace.
...
* From within the root directory of the current project, use the [dbx sync](https://dbx.readthedocs.io/en/latest/guides/python/devloop/mixed/#using-dbx-sync-repo-for-local-to-repo-synchronization) tool to copy code files from your local machine into the Repo by running
  `dbx sync repo --profile dbx_workshop-dev --source . --dest-repo your-repo-name`, where `your-repo-name` should be the last segment of the full repo name (`/Repos/username/your-repo-name`)
